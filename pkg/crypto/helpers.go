...
func EncodePrivateKey(key crypto.Signer) ([]byte, error) {
	...
	case ed25519.PrivateKey:
		...
		if err != nil {
			return nil, fmt.Errorf("can't marshal ed25519 private key: %w", err)
		}
		err = pem.Encode(&buffer, &pem.Block{
			Type:  "PRIVATE KEY",
			Bytes: derBytes,
		...
		if err != nil {
			return nil, fmt.Errorf("can't pem encode ed25519 private key: %w", err)
		}

	default:
		return nil, fmt.Errorf("unsupported signer key type %T", key)
	}
